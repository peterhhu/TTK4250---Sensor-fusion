...
class EKFSLAM:
    ...
    def h(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m = m - x[:2].reshape(2,1) - rotmat2d(x[2]) @ (self.sensor_offset).reshape(2,1)# Relative position of landmark to sensor on robot in world frame

        zpredcart = [Rot @ mi for mi in delta_m.T] # Predicted measurements in cartesian coordinates, beware sensor offset for VP
        
        zpred_r = [np.linalg.norm(mi) for mi in delta_m.T]# Ranges
        zpred_theta = [np.arctan2(mi[1],mi[0]) for mi in zpredcart]# Bearings
    ...
    def H(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m = m - x[:2].reshape(2,1) # Relative position of landmark to robot in world frame

        zc = delta_m - Rot @ (self.sensor_offset.reshape(2,1))# (2, #measurements), each measured position in cartesian coordinates like
        # [x coordinates;
        #  y coordinates]

        zpred = self.h(eta)# (2, #measurements), predicted measurements, like
        # [ranges;
        #  bearings]
        zr = zpred[::2]# Ranges
        ...
        assert (Hx.shape == (2 * numM, 3)
        ), "EKFSLAM.H: Wrong shape on calculated Hx"
        assert (Hm.shape == (2 * numM, 2 * numM)
        ), "EKFSLAM.H: Wrong shape on calculated Hm"
    ...
    def add_landmarks(
        ...
        for j in range(numLmk):
            ...
            lmnew[inds] = eta[:2] + rotmat2d(eta[2]) @ zj_cart + sensor_offset_world# Calculate position of new landmark in world frame
            ...
            Rall[inds, inds] = Gz @ self.R @ Gz.T # Transform measurement covariance from polar to cartesian coordinates

        assert len(lmnew) % 2 == 0, "SLAM.add_landmark: lmnew not even length"
        etaadded = np.concatenate([eta[:], lmnew]) # Append new landmarks to state vector
        Padded = la.block_diag(P, Gx @ P[:3,:3] @ Gx.T + Rall) # Block diagonal of P_new, see problem text in 1g) in graded assignment 3
        Padded[:n, n:] = P[:,:3] @ Gx.T  # Top right corner of P_new
        Padded[n:, :n] = Padded[:n, n:].T # Transpose of above
